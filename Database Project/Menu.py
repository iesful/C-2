...
while leave != "Y":
    ...
    if ActionChoice == 1:
        prompt = input("You have selected to create a new table would you like to continue (Y/N)? ")
        if prompt.upper == "Y":
    ...
    if ActionChoice == 2:
        prompt = input("You have selected to drop a table would you like to continue (Y/N)? ")
        if prompt.upper == "Y":
